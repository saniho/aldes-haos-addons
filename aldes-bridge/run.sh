...
if [ "$MQTT_PORT" != "8883" ]; then
  if [ -n "$BOX_IP" ]; then
    iptables -t nat -A PREROUTING -p tcp -s "$BOX_IP" --dport 8883 -j REDIRECT --to-port "$MQTT_PORT" 2>&1
    ip6tables -t nat -A PREROUTING -p tcp -s "$BOX_IP" --dport 8883 -j REDIRECT --to-port "$MQTT_PORT" 2>&1
    echo "[run.sh] iptables PREROUTING: 8883 -> $MQTT_PORT (source: $BOX_IP only)"
  else
    iptables -t nat -A PREROUTING -p tcp --dport 8883 -j REDIRECT --to-port "$MQTT_PORT" 2>&1
    ip6tables -t nat -A PREROUTING -p tcp --dport 8883 -j REDIRECT --to-port "$MQTT_PORT" 2>&1
    echo "[run.sh] iptables PREROUTING: 8883 -> $MQTT_PORT (all sources)"
  fi
  if [ -n "$BOX_IP" ]; then
    iptables -t nat -A OUTPUT -p tcp -s "$BOX_IP" --dport 8883 -j REDIRECT --to-port "$MQTT_PORT" 2>&1
  else
    iptables -t nat -A OUTPUT -p tcp --dport 8883 -j REDIRECT --to-port "$MQTT_PORT" 2>&1
  fi
